...
class Logger:
    ...
    def __init__(self, scrn=True, log_dir='', phase=''):
        ...
        if log_dir and phase:
            timestamp = '{}-{:-4d}-{:02d}-{:02d}-{:02d}-{:02d}-{:02d}'.format(phase, *localtime()[:6])
            self.log_path = os.path.join(log_dir, timestamp+'.log')
            ...
            self._writer = SummaryWriter(log_dir=os.path.join(log_dir,timestamp))
